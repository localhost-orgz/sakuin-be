...
export const getGoalByGoalId = async (req: Request<Params>, res: Response) => {
  try {
    const { uuid } = req.params;
    const goal = await goalService.getGoalByGoalId(uuid);
    if (!goal) {
...
export const updateGoal = async (req: Request<Params>, res: Response) => {
  try {
    const { uuid } = req.params;
    const updatedGoal = await goalService.updateGoal(uuid, req.body);
    if (!updatedGoal) {
      return res.status(404).json({ status: 'error', message: 'Goal not found' });
    }
    return res.status(200).json({
...
export const deleteGoal = async (req: Request<Params>, res: Response) => {
  try {
    const { uuid } = req.params;
    const deleted = await goalService.deleteGoal(uuid);
    if (!deleted) {
      return res.status(404).json({ status: 'error', message: 'Goal not found' });
    }
    return res.status(200).json({
